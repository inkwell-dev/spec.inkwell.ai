...
- [x] Article reader page (`/articles/[slug]`) — full article, premium gate *(the "Licensed by" badge was checked off in Phase 3 and did not exist: the reader page carried only a MARKETPLACE badge and a "Licensed on the Inkwell Marketplace" notice, and there was no publisher to name until `articles.publisher_id` shipped on 2026-09-07. **Built 2026-09-07** as an attribution rather than a badge — the article page names the publication inline after the author, and cards carry the same pairing. The history is kept rather than tidied away, because a box that read as done for two phases is the more useful record.)*
...
- [x] Content moderation on article publish: auto-flag violating content — **Groq only.** *Updated 2026-09-04: the OpenAI path was removed.* The chain used to prefer OpenAI's `/v1/moderations` when `OPENAI_API_KEY` was set, falling back to Groq; that endpoint is free but gated behind a paid credit balance, so on this project the key returned `credit_balance_exhausted` and the path could never run. The Groq classifier was separately dead for a period — `llama-3.1-8b-instant` was retired and the chain fails open, so publishes passed unmoderated with nothing in the logs. Now `groq/compound-mini`, with a liveness check over every model id.; there is no OpenAI key on this project (no international card), so it runs on Groq in practice. A flag writes a `pending` report and the article publishes regardless.
